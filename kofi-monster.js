...
const kofiWidgetOverlayConfig = {
    ...
    'floating-chat.core.position.bottom-left': 'position: fixed; bottom: 50px; left: 10px; width: 340px; height: 65px;',

    'floating-chat.cssId': '',
    'floating-chat.notice.text': 'ko-fi.com/%HANDLE%',
    'floating-chat.donatebutton.image': 'https://dtgxwmigmg3gc.cloudfront.net/imagery/assets/derivations/icon/512/512/true/eyJpZCI6IjkwMDg5ODFhYzFkMTk2YTgyNDE2MjBiN2U4OGZmMDIxIiwic3RvcmFnZSI6InB1YmxpY19zdG9yZSJ9?signature=1e6b9fa4501241b10525ed10b023d08333d0421caca1b9e7cce982f1bcffdfde',
    'floating-chat.donateButton.background-color': '#00b9fe',
    'floating-chat.donateButton.text': 'buy me a celsius',
    'floating-chat.donateButton.text-color': '#fff',
    'floating-chat.stylesheets': '["https://fonts.googleapis.com/css?family=DM+Sans:400,600,700,800&display=swap"]',
};
...
var kofiWidgetOverlayFloatingChatBuilder = kofiWidgetOverlayFloatingChatBuilder || function (config, _utils) {
    ...
    function createButtonContainerIframe(iframeId, mainStyleSheetFile) {
        ...
        var buttonBody = '<html style="color-scheme: auto;">' +
            '<head>' +
            ...
            `</head>` +
            `<body style="margin: 0; position: absolute; bottom: 0; background: transparent; font-weight:600 !important;">${htmlBody}</body>` +
            '</html>';
        ...
        var _timer = setInterval(function () {
            ...
            if (doc && doc.readyState == 'complete') {
                ...
                iframe.style = 'color-scheme: auto;';
            }
        }, 300);
    ...
    var write = function (parentElementId) {
        ...
        var iframeHtml = `<div class="${_topContainerWrapClass}" style="height: 0px; transition: all 0.3s ease 0s; opacity:0;">` +
            `<iframe allowtransparency="true" class="floatingchat-container" style="height: 0px; transition: all 0.6s ease 0s; opacity:0;" id="${iframeId}"></iframe>` +
            '</div>' +
            `<div class="${_topMobiContainerWrapClass}" style="height: 0px; transition: all 0.6s ease 0s; opacity:0;">` +
            `<iframe allowtransparency="true" class="floatingchat-container-mobi" style="height: 0px; transition: all 0.6s ease 0s; opacity:0;" id="${mobiIframeId}"></iframe>` +
            '</div>';
...
var kofiWidgetOverlay = kofiWidgetOverlay || (function () {
    ...
    const setConfigDefaults = function (config, widgetType, pId, instanceId) {
        ...
        config[widgetType + '.stylesheets'] = config[widgetType + '.stylesheets'] !== undefined ? config[widgetType + '.stylesheets'] : '["https://fonts.googleapis.com/css?family=DM+Sans:400,600,700,800&display=swap"]';
